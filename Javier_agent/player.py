...
def extract_featuresV3(pstate, soccer_state, opponent_state, team_id):
    ...
    kart_to_puck_distance = np.linalg.norm(puck_center-kart_center)

    #Is the puck in the between the player and the goal?
    #Create a bigger circle  than the radiusfor the puck
    rad = 3.2
    angle = np.linspace(0,2*np.pi,100)
    px = puck_center[0] + rad*np.cos(angle)
    py = puck_center[1] + rad* np.sin(angle)

    #Create a line in the opposite goal
    ...
    features = torch.tensor([kart_center[0], kart_center[1], kart_angle, kart_to_puck_angle,
        ...
        puck_center[0], puck_center[1], puck_to_goal_line[0], puck_to_goal_line[1],kart_to_puck_distance], dtype=torch.float32)
